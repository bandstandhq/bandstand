...
// routes.ts. Catches the case a hand-maintained mapping would silently
...
import { bandRoutePathToFile } from './routes';

describe('bandRoutePathToFile', () => {
  it('has exactly one entry per bandRouteShapes path — no more, no fewer', () => {
    const shapePaths = new Set(bandRouteShapes.map((r) => r.path));
    const componentPaths = new Set(Object.keys(bandRoutePathToFile));
